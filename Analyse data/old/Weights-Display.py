...
suptitle = 'Model: Topalidou-et-al-2015\n'
path = '../cython/'
import sys
...
protocol = input('Choose 1 for A-Protocol or 2 for B-Protocol\n')
if protocol == 1:
	suptitle += 'Protocol: A'
	inverse = input('\nThere is reverse of Probabilities during the simulation?\nChoose: \n0 for No \n1 for Yes\n')
	if inverse == 1:
		folder = '../Results/C-Results'
		inverse_trial = input('\nAfter how many trials is the reverse?\n')
...
plt.ylabel("Weights")
plt.xlabel("Trial number")
temp_title = 'Weights from\nCognitive to Associative Cortex'
plt.title(temp_title, fontsize=12)
plt.title(suptitle, loc='left', fontsize=12)
plt.title(title, loc='right', fontsize=12)
plt.ylim(0.495,0.52)
