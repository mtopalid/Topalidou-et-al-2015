...
rslt = folderRes + "NoGPi.npy"
rprt = folderRep + "NoGPi.txt"
experiment = Experiment(model = mdl,
						task = tsk,
						result = rslt,
						report = rprt,
                        n_session = 25, n_block = 1, seed = 1)
records = experiment.run(session, "Piron Protocol without GPi")
...
D2_std = D2.std()
plt.plot(X, D2_mean, c='r', lw=2)
plt.plot(X, D2_mean + D2_std, c='r', lw=.5)
plt.plot(X, D2_mean - D2_std, c='r', lw=.5)
plt.fill_between(X, D2_mean + D2_std, D2_mean - D2_std, color='r', alpha=.1)
#
# plt.text(n, D2_mean[-1], "%.2f" % D2_mean[-1],
...
X += n+1
D3_mean = D3.mean(axis=0)
D3_std = D3.std()
plt.plot(X, D3_mean, c='r', lw=2)
plt.plot(X, D3_mean + D3_std, c='r', lw=.5)
plt.plot(X, D3_mean - D3_std, c='r', lw=.5)
plt.fill_between(X, D3_mean + D3_std, D3_mean - D3_std, color='r', alpha=.1)
#
# plt.text(n, D3_mean[-1], "%.2f" % D3_mean[-1],
#          ha="left", va="center", color="r")
...
#
#
# fl = folderFig + "histogram-NoGpi.pdf"
# plt.savefig(fl)
#
#
#
