...
def session(exp):
    ...
    g3 = exp.model["CTX:cog → STR:ass"].gain
    g4 = exp.model["CTX:mot → STR:ass"].gain

    exp.model["CTX:cog → STR:ass"].gain = 0
    exp.model["CTX:mot → STR:ass"].gain = 0
...
rslt = folderRes + "protocol-NoStrAss.npy"
rprt = folderRep + "protocol-NoStrAss.txt"
experiment = Experiment(model = mdl,
						task = tsk,
						result = rslt,
						report = rprt,
                        n_session = 25, n_block = 2, seed=377)
records = experiment.run(session, "Thomas Protocol No Str Ass")
...
fl = folderFig + "protocol-histogram-ass.pdf"
