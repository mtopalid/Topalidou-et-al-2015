...
def session(exp):
    ...
    g2 = exp.model["CTX:mot → STR:mot"].gain

    exp.model["CTX:mot → STR:mot"].gain = 0
...
rslt = folderRes + "protocol-NoStrMot.npy"
rprt = folderRep + "protocol-NoStrMot.txt"
experiment = Experiment(model = mdl,
						task = tsk,
						result = rslt,
						report = rprt,
                        n_session = 25, n_block = 2, seed=377)
records = experiment.run(session, "Thomas Protocol No Str Moto")
...
fl = folderFig + "protocol-histogram-mot.pdf"
