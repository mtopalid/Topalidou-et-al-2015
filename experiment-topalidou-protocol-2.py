...
P = np.squeeze(records["best"][:,2,:])*100
P = P.mean(axis=len(P.shape)-1)
print("D3 : %.3f ± %.3f" % (P.mean(), P.std()))
P = np.squeeze(records["best"][:,2,:25])*100
P = P.mean(axis=len(P.shape)-1)
...
P = P.mean(axis=len(P.shape)-1)
Dend = P.mean(), P.std()
D3_mean, D3_std = (Dstr[0], Dend[0]), (Dstr[1], Dend[1])
...
fl = folderFig + "protocol-2-histogram.pdf"
